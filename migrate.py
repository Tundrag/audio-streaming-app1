...
Revision ID: add_new_features_202403
...
down_revision = 'add_new_features_202410' 
branch_labels = None
depends_on = None

def upgrade():
    # Create enum types first
    op.execute("CREATE TYPE contentvisibility AS ENUM ('public', 'private', 'tier_only', 'scheduled')")
    op.execute("CREATE TYPE auditlogtype AS ENUM ('create', 'update', 'delete', 'login', 'permission_change', 'content_access')")
    op.execute("CREATE TYPE notificationtype AS ENUM ('comment', 'reply', 'mention', 'new_content', 'tier_update', 'system')")
    ...
    op.add_column('users', sa.Column('social_links', sa.JSON(), server_default='{}', nullable=True))
    op.add_column('users', sa.Column('preferences', sa.JSON(), server_default='{}', nullable=True))
    op.add_column('users', sa.Column('notification_settings', sa.JSON(), server_default='{}', nullable=True))
    ...
    op.create_table('campaign_tiers',
        ...
        sa.Column('benefits', sa.JSON(), server_default='{}', nullable=True),
        sa.Column('downloads_allowed', sa.Integer(), server_default='0', nullable=False),
        sa.Column('custom_perks', sa.JSON(), server_default='{}', nullable=True),
    ...
    op.create_table('notifications',
        ...
        sa.Column('uuid', sa.String(), nullable=False),
        sa.Column('user_id', sa.Integer(), nullable=False),
        sa.Column('type', sa.Enum('comment', 'reply', 'mention', 'new_content', 'tier_update', 'system', 
    ...
    # Create UUID values for existing rows
    op.execute("""
        UPDATE users 
...
def downgrade():
    ...
    op.execute("DROP TYPE IF EXISTS auditlogtype")
    op.execute("DROP TYPE IF EXISTS contentvisibility")
